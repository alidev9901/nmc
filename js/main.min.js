...
let featuresSlider;
	...
	const breakpointCheckerFeatures = function() {
		...
		if ( breakpoint.matches === true ) {
			...
			return enableSwiperFeatures();

		}

	};

	const enableSwiperFeatures = function() {
	...
	breakpoint.addListener(breakpointCheckerFeatures);

	// kickstart
	breakpointCheckerFeatures();
...
document.addEventListener('click', function(event) {
  ...
  const materialList = document.querySelector('.materials__list');

  if (window.innerWidth >= 1170  && materialList && !materialList.contains(event.target)) {
    $(materialList).find('.materials__list-item--active').removeClass('materials__list-item--active').find('.materials__submenu').slideToggle()
  }
...
$('.materials__list-item-button').on('click', function() {


    if( $(this).closest('.materials__list').find('.materials__list-item--active').length ) {

      $(this).closest('.materials__list').find('.materials__list-item--active').not($(this).closest('.materials__list-item')).removeClass('materials__list-item--active').find('.materials__submenu').slideToggle()
  

    }

    $(this).closest('.materials__list-item').toggleClass('materials__list-item--active')
    $(this).closest('.materials__list-item').find('.materials__submenu').slideToggle();
